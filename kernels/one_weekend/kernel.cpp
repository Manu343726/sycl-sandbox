...
extern "C" void init_kernel(sycl::queue* q,int,int,const void* params,size_t){
    ...
    int n=(int)p[P_N];
    rt::float3 gc; memcpy(&gc,p+P_GC,12); memcpy(&g_bg,p+P_BG,12);
    if(g_objs){sycl::free(g_objs,*q);g_objs=nullptr;}
    srand(42);
    auto* o=new rt::Object[n+4+3];
    int c=0;

    o[c++]=rt::Object::make_sphere({0,-1000,0},1000,rt::Lambertian(gc));

    for(int k=0;k<n;){
        float x=-10+20*rnd(),z=-10+20*rnd();
        rt::float3 cen={x,0.2f,z};
        if(rt::len(cen)<=0.9f)continue;
        rt::float3 col={rnd()*rnd(),rnd()*rnd(),rnd()*rnd()};
        float d=rnd();
        if(d<0.6f)      o[c++]=rt::Object::make_sphere(cen,0.2f,rt::Lambertian(col));
        else if(d<0.85f) o[c++]=rt::Object::make_sphere(cen,0.2f,rt::Metal(col,0.5f*rnd()));
        else             o[c++]=rt::Object::make_sphere(cen,0.2f,rt::Dielectric(1.5f));
        k++;
    }
    o[c++]=rt::Object::make_sphere({4,1,0},1,rt::Metal({0.7f,0.6f,0.5f},0));
    o[c++]=rt::Object::make_sphere({-4,1,0},1,rt::Lambertian({0.4f,0.2f,0.1f}));
    o[c++]=rt::Object::make_sphere({0,1,0},1,rt::Dielectric(1.5f));

    g_objs=sycl::malloc_device<rt::Object>(c,*q);
    q->memcpy(g_objs,o,c*sizeof(rt::Object)).wait();
    g_n=c; delete[] o;
}
...
extern "C" void render_kernel(sycl::queue* q,int w,int h,const void* p,void* accum,int si){
    rt::render_main(q,w,h,(const float*)p,(float*)accum,si,g_objs,g_n,
        [bg=g_bg](const rt::Ray& ray)->rt::float3{
            return rt::lerp({1,1,1},bg,0.5f*(ray.dir.y+1));
        });
}
