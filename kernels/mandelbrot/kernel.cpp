...
static ParamMeta params_meta[] = {
    { "center_x", "Real component of center",
      ParamType::FLOAT, .range = { .f = { -2.0f, 2.0f, 0.01f } }, .default_f = -0.5f },
    { "center_y", "Imaginary component of center",
      ParamType::FLOAT, .range = { .f = { -2.0f, 2.0f, 0.01f } }, .default_f = 0.0f },
    { "zoom", "Zoom level (larger = more zoomed in)",
      ParamType::FLOAT, .range = { .f = { 0.1f, 100.0f, 0.1f } }, .default_f = 3.0f },
    { "max_iterations", "Maximum iterations per point",
      ParamType::INT, .range = { .i = { 10, 10000, 10 } }, .default_i = 300 },
};

enum ParamIdx : int {
    P_CENTER_X=0, P_CENTER_Y=1, P_ZOOM=2, P_MAX_ITER=3,
};

static const char* source_files[] = { "kernel.cpp", nullptr };
static KernelDesc desc = {
    "mandelbrot", "Colored Mandelbrot fractal",
    4, params_meta, 0, 1, 2, source_files
};
...
extern "C" void render_kernel(sycl::queue* q, int w, int h,
                               const void* params, void* accum, int) {
    ...
    q->parallel_for(sycl::range<2>{(size_t)h, (size_t)w},
        ...
        if (iter == max_iter) {
            col_r = 0.0f; col_g = 0.0f; col_b = 0.0f;
        } else {
            ...
            switch (hi % 6) {
                case 0: col_r=v; col_g=tt; col_b=p; break;
                case 1: col_r=q; col_g=v; col_b=p; break;
                case 2: col_r=p; col_g=v; col_b=tt; break;
                case 3: col_r=p; col_g=q; col_b=v; break;
                case 4: col_r=tt; col_g=p; col_b=v; break;
                default: col_r=v; col_g=p; col_b=q; break;
            }
        }
