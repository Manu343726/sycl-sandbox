...
The host builds ImGui parameter controls and buffer layout from the YAML
scene file's `ParamDescriptor[]` — NOT from the kernel descriptor.
Parameters (including standard params like SPP, bounces, camera, tick, time)
are all declared in the YAML and auto-generated by `load_scene_descriptor()`.
...
- **TracyClient** is fetched via `FetchContent` and linked into the `sycl-sandbox`
