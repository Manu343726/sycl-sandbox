...
public:
    ...
    ///
    /// \param ctx per-call kernel context: records a "hit_box" profiler
    ///        zone (decimated by work-item id) and reports the hit test
    ///        to the trace collector; forwarded to the per-face quad hit
    ///        tests, so each face also reports.
    optional<HitRecord> hit(const Ray &ray, float t_min, float t_max,
                            const Context &ctx = Context{}) const {
        PROFILER_FUNCTION();
        ctx.collector.on_hit_test(HittableType::Box, ray, t_min, t_max);
