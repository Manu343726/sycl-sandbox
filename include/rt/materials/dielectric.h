...
public:
    ...
    std::optional<ScatterRecord> scatter(const Ray& in, const HitRecord& rec, RNG& rng) const {
        ...
        if (refract(in.dir, outward_n, eta_ratio, refracted) && rng.next() >= schlick(cos, ir))
            return ScatterRecord{{1,1,1}, Ray{rec.p, refracted}};
        else
            return ScatterRecord{{1,1,1}, Ray{rec.p, reflect(norm(in.dir), rec.normal)}};
    }

    float3 emit(const HitRecord&) const { return {0,0,0}; }
};

inline Dielectric dielectric(float ir) { return Dielectric(ir); }
